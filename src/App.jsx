...
export default function App() {
  ...
  const eyebrow = { fontFamily: "Manrope", fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase", fontSize: 20, color: COLORS.gold };
...
function Field({ label, children }) {
  return (
    <div style={{ marginBottom: 12 }}>
      <div style={{ fontSize: 21, fontWeight: 700, marginBottom: 5, color: COLORS.muted }}>{label}</div>
      {children}
    </div>
  );
}
...
function BackRow({ onClick }) {
  return (
    <button onClick={onClick} style={{ background: "none", border: "none", display: "flex", alignItems: "center", gap: 4, padding: "4px 0 10px", color: COLORS.muted, fontWeight: 600, fontSize: 22 }}>
  ...
  if (step === 0) {
    return (
      <div style={{ maxWidth: 500, margin: "0 auto", padding: "28px 16px 60px" }} className="fade-in">
        <div style={{ ...display, fontSize: 48, fontWeight: 600, textAlign: "center", marginBottom: 6 }}>What can we get you?</div>
        <div style={{ textAlign: "center", color: COLORS.muted, fontSize: 24, marginBottom: 24 }}>Raw Michigan honey, infused with real flavor.</div>
        ...
        <button onClick={() => { setOrderType("pickup"); setStep(1); }} style={{
          ...
          <div>
            <div style={{ ...display, fontWeight: 700, fontSize: 30 }}>Order for Pickup</div>
            <div style={{ fontSize: 22, color: COLORS.muted, marginTop: 4, lineHeight: 1.5 }}>
        ...
        <button onClick={() => { setOrderType("event"); setStep(1); }} style={{
          ...
          <div>
            <div style={{ ...display, fontWeight: 700, fontSize: 30 }}>Request Honey for an Event</div>
            <div style={{ fontSize: 22, color: COLORS.muted, marginTop: 4, lineHeight: 1.5 }}>
  ...
  if (orderType === "event") {
    ...
    return (
      <div style={{ maxWidth: 500, margin: "0 auto", padding: "20px 16px 60px" }} className="fade-in">
        ...
        <div style={{ ...display, fontSize: 42, fontWeight: 700, margin: "6px 0 16px" }}>Tell us about your event</div>
    ...
    <div style={{ maxWidth: 500, margin: "0 auto", padding: "20px 16px 60px", position: "relative" }}>
      ...
      {step === 1 && (
        <div className="fade-in">
          ...
          <div style={{ ...display, fontSize: 42, fontWeight: 700, margin: "6px 0 16px" }}>What are you taking home?</div>

          <div style={cardStyle}>
            <div style={{ fontSize: 21, fontWeight: 700, color: COLORS.muted, marginBottom: 6 }}>Size</div>
            ...
            <div style={{ fontSize: 21, color: COLORS.muted, marginBottom: 14 }}>{sizeMeta?.blurb}</div>

            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
              <div style={{ fontSize: 21, fontWeight: 700, color: COLORS.muted }}>Honey type</div>
              <button onClick={() => setShowTypeInfo(true)} style={{ background: "none", border: "none", color: COLORS.gold, display: "flex", alignItems: "center", gap: 4, fontSize: 21, fontWeight: 700 }}>
            ...
            <div style={{ marginTop: 18 }}>
              ...
              {cart.map((i) => (
                <div key={i.lineId} style={{ ...cardStyle, marginTop: 8, display: "flex", justifyContent: "space-between", alignItems: "center", padding: 12 }}>
                  <div>
                    <div style={{ fontWeight: 700, fontSize: 24 }}>{i.qty}× {i.sizeLabel} — {i.flavorName}</div>
                    <div style={{ fontSize: 21, color: COLORS.muted }}>{i.honeyType === "spun" ? "Spun" : "Regular"} · ${(i.unitPrice * i.qty).toFixed(2)}</div>
                  </div>
              ...
              {qty4ozInCart > 0 && qty4ozInCart % DEAL_QTY_4OZ !== 0 && (
                <div style={{ fontSize: 21, color: COLORS.gold, marginTop: 8 }}>
                  Add {DEAL_QTY_4OZ - (qty4ozInCart % DEAL_QTY_4OZ)} more 4oz jar(s) to hit the next 3-for-$20 deal.
                </div>
              )}
              <div style={{ display: "flex", justifyContent: "space-between", marginTop: 10, fontWeight: 800, fontSize: 26 }}>
      ...
      {step === 2 && (
        <div className="fade-in">
          ...
          <div style={{ ...display, fontSize: 42, fontWeight: 700, margin: "6px 0 16px" }}>Where will you pick up?</div>
          ...
          {markets.map((m) => (
            <button key={m.id} onClick={() => { setSelectedMarketId(m.id); setSelectedSlot(null); }} style={{
              ...
              {m.address && <div style={{ fontSize: 22, color: COLORS.muted, display: "flex", alignItems: "center", gap: 6, marginTop: 2 }}><MapPin size={17} /> {m.address}</div>}
            </button>
          ))}
      ...
      {step === 3 && selectedMarket && (
        <div className="fade-in">
          ...
          <div style={{ ...display, fontSize: 42, fontWeight: 700, margin: "6px 0 4px" }}>When will you swing by?</div>
          <div style={{ fontSize: 22, color: COLORS.muted, marginBottom: 16 }}>{selectedMarket.name} · {fmtDate(selectedMarket.date)}</div>
            ...
            {generateSlots(selectedMarket).map((slot) => {
              ...
              return (
                <button key={slot} disabled={full} onClick={() => setSelectedSlot(slot)} style={{
                  ...
                  <div style={{ fontSize: 20, marginTop: 3, fontWeight: 700 }}>{full ? "Full" : `${remaining} spot${remaining === 1 ? "" : "s"} left`}</div>
                </button>
              );
            })}
          </div>
      ...
      {step === 4 && selectedMarket && (
        <div className="fade-in">
          ...
          <div style={{ ...display, fontSize: 42, fontWeight: 700, margin: "6px 0 16px" }}>Who's picking up?</div>

          <div style={{ ...cardStyle, marginBottom: 14 }}>
            <div style={{ fontSize: 20, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.08em", color: COLORS.muted, marginBottom: 4 }}>Order</div>
          ...
          <div style={{ fontSize: 21, color: COLORS.muted, textAlign: "center", marginTop: 8 }}>Pay with cash or card when you pick up — nothing charged now.</div>
        </div>
      )}
      ...
      {step === 5 && confirmedOrder && (
        <div className="fade-in" style={{ textAlign: "center", paddingTop: 10 }}>
          ...
          <div style={{ ...cardStyle, textAlign: "left" }}>
            <div style={{ fontSize: 20, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.08em", color: COLORS.muted, marginBottom: 4 }}>Pickup</div>
      ...
      {showUpsell && builderFlavor && (
        <div style={{ position: "fixed", inset: 0, background: "#000000aa", display: "flex", alignItems: "center", justifyContent: "center", padding: 20, zIndex: 50 }}>
          <div style={{ ...cardStyle, maxWidth: 380, width: "100%", animation: "popIn 0.2s ease both" }}>
            ...
            <div style={{ ...display, fontSize: 32, fontWeight: 700, marginTop: 8 }}>Wait — mix &amp; match sale!</div>
            <div style={{ fontSize: 22, color: COLORS.muted, marginTop: 6, lineHeight: 1.5 }}>
            ...
            <button onClick={() => setShowUpsell(false)} style={{ background: "none", border: "none", color: COLORS.muted, fontSize: 21, width: "100%", marginTop: 8, padding: 6 }}>
      ...
      {showTypeInfo && (
        <div style={{ position: "fixed", inset: 0, background: "#000000aa", display: "flex", alignItems: "center", justifyContent: "center", padding: 20, zIndex: 50 }}>
          <div style={{ ...cardStyle, maxWidth: 380, width: "100%", animation: "popIn 0.2s ease both" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div style={{ ...display, fontSize: 32, fontWeight: 700 }}>Regular vs. Spun</div>
            ...
            {HONEY_TYPES.map((t) => (
              <div key={t.id} style={{ marginTop: 12 }}>
                <div style={{ fontWeight: 700, fontSize: 24, color: COLORS.gold }}>{t.label}</div>
                <div style={{ fontSize: 22, color: COLORS.muted, marginTop: 3, lineHeight: 1.5 }}>{t.blurb}</div>
              </div>
            ))}
  ...
  if (!unlocked) {
    return (
      <div style={{ maxWidth: 360, margin: "60px auto", padding: "0 20px", textAlign: "center" }}>
        ...
        <div style={{ ...display, fontSize: 32, fontWeight: 700, marginBottom: 10 }}>Vendor login</div>
        ...
        <div style={{ fontSize: 20, color: COLORS.muted, marginTop: 12 }}>Passcode is set in src/App.jsx — change ADMIN_PASSCODE, then rebuild and redeploy.</div>
      </div>
    );
  }
              ...
              return (
                <div key={m.id} style={{ ...cardStyle, marginBottom: 8, display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div>
                    ...
                    <div style={{ fontSize: 21, color: COLORS.muted }}>{fmtDate(m.date)} · {fmtTime(m.startTime)}–{fmtTime(m.endTime)}</div>
                    <div style={{ fontSize: 21, color: COLORS.muted }}>{totalBooked} reservation{totalBooked === 1 ? "" : "s"} so far</div>
                  </div>
      ...
      {adminTab === "flavors" && (
        <div>
          ...
          <div style={{ ...cardStyle, margin: "8px 0 18px" }}>
            ...
            <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 22, marginBottom: 10, color: COLORS.cream }}>
          ...
          <div style={{ fontSize: 21, color: COLORS.muted, margin: "4px 0 8px" }}>Toggle Featured to spotlight a flavor for customers. Toggle Active off to hide it without deleting it (handy for rotating flavors out for the season).</div>
          {flavors.map((f) => (
            <div key={f.id} style={{ ...cardStyle, marginBottom: 8, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                <div style={{ fontWeight: 700, fontSize: 24 }}>{f.name}</div>
                <div style={{ fontSize: 20, color: COLORS.muted, textTransform: "capitalize" }}>{f.category}{!f.active ? " · hidden" : ""}</div>
              </div>
          ...
          <div style={{ marginTop: 8 }}>
            {orders.map((o) => (
              <div key={o.id} style={{ ...cardStyle, marginBottom: 8, opacity: o.pickedUp ? 0.5 : 1 }}>
                ...
                <div style={{ fontSize: 21, color: COLORS.muted }}>{o.email} · {o.phone}</div>
                <div style={{ fontSize: 21, marginTop: 4 }}>{o.marketName} · {fmtDate(o.marketDate)} at {fmtTime(o.slot)}</div>
                <div style={{ fontSize: 21, marginTop: 4, color: COLORS.muted }}>
            ...
            {eventRequests.map((r) => (
              <div key={r.id} style={{ ...cardStyle, marginBottom: 8, opacity: r.status === "booked" ? 0.6 : 1 }}>
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  ...
                  <div style={{ fontSize: 20, fontWeight: 700, textTransform: "uppercase", color: COLORS.orange }}>{r.status}</div>
                </div>
                <div style={{ fontSize: 21, color: COLORS.muted }}>{r.email} · {r.phone}</div>
                <div style={{ fontSize: 22, marginTop: 6 }}><strong>Event:</strong> {r.eventType || "—"} {r.eventDate ? `· ${fmtDate(r.eventDate)}` : ""}</div>
                <div style={{ fontSize: 22, marginTop: 2 }}><strong>Quantity:</strong> {r.quantityEstimate || "—"}</div>
                <div style={{ fontSize: 22, marginTop: 2 }}><strong>Sizes:</strong> {r.preferredSizes || "no preference"}</div>
                <div style={{ fontSize: 22, marginTop: 2 }}><strong>Flavors:</strong> {r.preferredFlavors || "no preference"}</div>
                {r.budgetNote && <div style={{ fontSize: 22, marginTop: 2 }}><strong>Budget:</strong> {r.budgetNote}</div>}
                {r.notes && <div style={{ fontSize: 21, marginTop: 4, fontStyle: "italic", color: COLORS.muted }}>"{r.notes}"</div>}
